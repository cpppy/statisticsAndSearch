...
int main()
{
    ...
    while(timeDis<100)    //累加到100就认为文件不会再被修改，整个过程结束
    {
        ...
        if(sIn.size() == 0)
        {
            ...
            tmplog.push_back("["+to_string(countBag)+"]"+"   array from readTXT() was empty!   ");
            break;
        }
       ...
       //获得文本中第一行里的时间，用以判断文本是否被修改
        ...
        else
        {
            ...
            cout<<tmpStr<<endl;
            tmplog.push_back("    Not Found timeStrPos, can not get timeStr !    "+tmpStr);
        }
        ...
        if(timeStr != preTimeStr)    //判断文件是否被修改
        {
            ...
            cout<<"["<<countBag<<"]";
            tmplog.push_back("dataBagID : "+to_string(countBag)+"\t"+"HeadTime :  "+timeStr);
            ...
            for(int i =10;i<sIn.size()-10;++i)
            {
                ...
                if(tmpStr[4]=='2')
                {
                    ...
                    else
                    {
                        ...
                        //cout<<tmpStr<<endl;
                        //break;
                        tmplog.push_back("|||    ---TPBE---was not found in this line !   ||| "+ tmpStr);
                    }

                    //statistic and restore
                    if(time2hm(timess) != preData[IDindex][1] && getCol(preData[IDindex][1])>0)    //当本次时间的分钟发生改变时，则选取preData的数据作为上一分钟的数据写入resOut中
                    {
                    ...
                    if(IDindex>1173)
                    {
                        ...
                        cout<<tmpStr<<endl;
                        tmplog.push_back(" |||   IDindex = "+to_string(IDindex)+"\t"+"new MD002 come out |||");
                        break;
                    }
                }
                logging.push_back(tmplog);
                tmplog.clear();
            }
            IDindex=0;
...
void restoreResult(vector<vector<string> > resOut)
{
    if(resOut[0].size()<120)
    {
        ofstream outf("logging.txt",ios::out);
        if(!outf)
        {
