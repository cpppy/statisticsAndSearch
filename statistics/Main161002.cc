...
void log2file(vector<string> tmplog);
        ...
        if(timeStr != preTimeStr)    //判断文件是否被修改
        {
            ...
            for(int i =10;i<sIn.size()-10;++i)
            {
                ...
                if(tmpStr[4]=='2')
                {
                    ...
                    if(time2hm(timess)=="11:29" || time2hm(timess)=="11:30" || time2hm(timess)=="14:58" || time2hm(timess)=="14:59" || time2hm(timess)=="15:00" )
                    {
                        tmplog.push_back(tmpStr);
                    }
...
void log2file(vector<string> tmplog)
{
    ofstream outf2("logging.txt",ios::out);
    if(!outf2)
    {
        cout<<"fail to open file 'logging.txt'"<<endl;
        exit(-1);
    }
